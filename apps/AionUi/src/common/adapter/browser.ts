...
if (win.electronAPI) {
  ...
  const connect = () => {
    ...
    socket.addEventListener('message', (event: MessageEvent) => {
      ...
      try {
        ...
        if (payload.name === 'auth-expired') {
          console.warn('[WebSocket] Received auth-expired event, reconnecting without login redirect');
          socket?.close();
          return;
        }
    ...
    socket.addEventListener('close', (event: CloseEvent) => {
      ...
      if (event.code === 1008) {
        console.warn('[WebSocket] Connection rejected with code 1008, retrying without login redirect');
        scheduleReconnect();
        return;
      }
