...
export const useWebCliTerminalStore = defineStore('webcliTerminal', {
  ...
  actions: {
    ...
    consumePendingHistoryRequest(message) {
      const reqId = String(message?.req_id || '').trim();
      if (!reqId) {
        return false;
      }
      const pending = this.pendingHistoryRequests[reqId];
      if (!pending) {
        return false;
      }
      delete this.pendingHistoryRequests[reqId];
      pending.resolve(cloneMessage(message));
      return true;
    },
    ...
    ensureStreamState(instanceId) {
      ...
      if (!this.streamStates[id]) {
        this.streamStates[id] = {
          ...
          newestHistoryCursor: 'h-1',
          snapshotHasAnsi: false,
          snapshotAnsiTruncated: false,
          syncInFlight: false,
          activeReqId: '',
          pendingRaw: [],
          syncTimeout: null,
    ...
    markSnapshotReady(instanceId, message) {
      ...
      stream.historyAvailable = Math.max(0, Number(message?.history?.available) || 0);
      stream.historyExhausted = false;
      stream.newestHistoryCursor = String(message?.history?.newest_cursor || 'h-1').trim() || 'h-1';
      stream.snapshotHasAnsi = String(message?.ansi || '').length > 0;
      stream.snapshotAnsiTruncated = message?.ansi_truncated === true;
    },
    ...
    async requestScreenSync(instanceId, reason = 'manual') {
      ...
      await this.connection.invoke('RequestSync', {
        instanceId: id,
        type: 'screen',
        reqId: stream?.screenSyncReqId || `screen-sync-${Date.now()}`
      });
    },

    async requestHistoryChunk(instanceId, options = {}) {
      const id = String(instanceId || '').trim();
      if (!id || !this.connection || !this.wsConnected) {
        return null;
      }

      const before = String(options?.before || 'h-1').trim() || 'h-1';
      const limit = Math.max(1, Math.min(200, Number(options?.limit) || 200));
      const reqId = String(options?.reqId || `history-sync-${Date.now()}-${Math.floor(Math.random() * 100000)}`);

      const chunk = await new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          delete this.pendingHistoryRequests[reqId];
          reject(new Error(`history sync timeout: ${reqId}`));
        }, SCREEN_SYNC_TIMEOUT_MS);

        this.pendingHistoryRequests[reqId] = {
          resolve: (message) => {
            clearTimeout(timer);
            resolve(message);
          },
        ...
        this.connection.invoke('RequestSync', {
          instanceId: id,
          type: 'history',
          reqId,
          before,
    ...
    async getPlainTextHistory(instanceId, options = {}) {
      const id = String(instanceId || this.selectedInstanceId || '').trim();
      if (!id) {
        return [];
      }

      const stream = this.ensureStreamState(id);
      const historyAvailable = Math.max(0, Number(stream?.historyAvailable) || 0);
      if (!stream || historyAvailable <= 0) {
        return [];
      }

      const lines = [];
      let remaining = historyAvailable;
      let before = String(stream.newestHistoryCursor || 'h-1').trim() || 'h-1';
      let chunkIndex = 0;

      while (remaining > 0 && chunkIndex < 25) {
        const limit = Math.max(1, Math.min(Number(options?.chunkLimit) || 200, remaining));
        const chunk = await this.requestHistoryChunk(id, {
          before,
          limit,
          reqId: `history-sync-${Date.now()}-${chunkIndex}`
        });

        const chunkLines = Array.isArray(chunk?.lines)
          ? chunk.lines.map((line) => {
              const segs = Array.isArray(line?.segs) ? line.segs : [];
              return segs
        ...
        const nextBeforeValue = String(chunk?.next_before || '').trim() || this.streamStates[id]?.newestHistoryCursor;
        this.streamStates[id].newestHistoryCursor = nextBeforeValue;
        this.streamStates[id].historyExhausted = chunk?.exhausted === true;
        if (!nextBeforeValue || nextBeforeValue === before) {
          break;
        }
        ...
        if (this.streamStates[id]?.historyExhausted === true) {
          break;
        }
      }
    ...
    processIncomingMessage(message) {
      ...
      if (message.type === 'term.history.chunk') {
        if (this.consumePendingHistoryRequest(message)) {
          return [];
        }
        const stream = this.ensureStreamState(instanceId);
        if (stream) {
          stream.newestHistoryCursor = String(message?.next_before || stream.newestHistoryCursor || 'h-1').trim() || 'h-1';
          stream.historyExhausted = message?.exhausted === true;
        }
        return [message];
      }
    ...
    async ensureConnection() {
      ...
      connection.onclose(() => {
        ...
        this.joinedInstanceIds = [];
        for (const reqId of Object.keys(this.pendingHistoryRequests)) {
          this.pendingHistoryRequests[reqId]?.reject?.(new Error('connection closed'));
        }
        this.pendingHistoryRequests = {};
        for (const id of Object.keys(this.streamStates)) {
          const stream = this.streamStates[id];
          if (stream?.syncTimeout) {
            clearTimeout(stream.syncTimeout);
    ...
    async terminateInstance(instanceId) {
      ...
      const endpoint = nodeId
        ...
        : buildApiPath(`/api/instances/${encodeURIComponent(id)}`);
      const response = await fetch(endpoint, {
        method: 'DELETE'
      });
      if (!response.ok) {
