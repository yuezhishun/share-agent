...
public sealed class TerminalEventRelayV2
{
    private readonly IHubContext<TerminalHubV2> _hub;
    private readonly ConcurrentDictionary<string, SemaphoreSlim> _instanceGates = new(StringComparer.Ordinal);

    public TerminalEventRelayV2(InstanceManager manager, IHubContext<TerminalHubV2> hub)
    {
        _hub = hub;
    ...
    private async Task EnqueueAsync(string instanceId, object payload)
    {
        ...
        try
        {
            await _hub.Clients.Group(TerminalHubV2.BuildInstanceGroup(instanceId)).SendAsync("TerminalEvent", payload);
        }
        catch
        {
        }
