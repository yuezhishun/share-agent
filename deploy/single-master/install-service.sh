...
SERVICE_NAME="${SERVICE_NAME:-terminal-gateway-single-master.service}"
ENV_FILE="${ENV_FILE:-/etc/default/terminal-gateway-single-master}"
LOG_DIR="${LOG_DIR:-/www/wwwlogs}"
HOST_VALUE="${HOST_VALUE:-127.0.0.1}"
PORT_VALUE="${PORT_VALUE:-7300}"
FILES_BASE_PATH="${FILES_BASE_PATH:-/home/yueyuan}"
SETTINGS_STORE_FILE="${TERMINAL_SETTINGS_STORE_FILE:-/tmp/pty-agent-terminal-settings-single-master.json}"
PROFILE_STORE_FILE="${TERMINAL_PROFILE_STORE_FILE:-/tmp/pty-agent-terminal-profiles-single-master.json}"
...
[Unit]
Description=Terminal Gateway Single Master
...
Restart=always
RestartSec=2
StandardOutput=append:${LOG_DIR}/terminal-gateway-single-master.out.log
StandardError=append:${LOG_DIR}/terminal-gateway-single-master.err.log
...
echo "single-master service installed"
